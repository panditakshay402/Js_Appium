...
function normalizeConfig(config) {
    ...
    const isSchemaTypeObject = (schemaObj) => Boolean(schemaObj?.properties ||
        schemaObj?.type === 'object');
    const normalize = (rootConfig, section) => {
        const obj = section === undefined
            ? rootConfig
            : (0, utils_1.getPath)(rootConfig, section, rootConfig);
...
function yamlLoader(filepath, content) {
    try {
        ...
        throw new Error(`The YAML config at '${filepath}' cannot be loaded. Original error: ${e.message}`, { cause: e });
    }
}
/**
...
function jsonLoader(filepath, content) {
    ...
    try {
        ...
        throw new Error(`The JSON config at '${filepath}' cannot be loaded. Original error: ${e.message}`, { cause: e });
    }
}
/**
