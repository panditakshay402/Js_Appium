...
class AppiumSchema {
    ...
    static isSupportedSchemaType(schema) {
        return support_1.util.isPlainObject(schema) && schema.$async !== true;
    }
    /**
    ...
    static _instantiateAjv() {
        ...
        for (const keyword of Object.values(keywords_1.keywords)) {
    ...
    async finalize() {
        ...
        const ajv = this.#ajv;
        const baseSchema = structuredClone(schema_1.AppiumConfigJsonSchema);
        ...
        addArgSpecs((0, utils_1.omitKeys)(baseSchema.properties.server.properties, [constants_1.DRIVER_TYPE, constants_1.PLUGIN_TYPE]));
    ...
    async registerSchema(extType, extName, schema) {
        ...
        const normalizedExtName = (0, utils_1.kebabCase)(extName);
        if (this.hasRegisteredSchema(extType, normalizedExtName)) {
            if (support_1.util.isEqual(this.#registeredSchemas[extType].get(normalizedExtName), schema)) {
                return;
            }
    ...
    getDefaults(flatten = true) {
        ...
        const reducer = flatten
            ...
            : (defaults, { defaultValue, dest }) => {
                if (defaultValue !== undefined) {
                    (0, utils_1.setPath)(defaults, dest, defaultValue);
                }
                return defaults;
            };
        const retval = {};
        ...
        for (const { properties, prefix } of stack) {
            ...
            for (const [key, value] of pairs) {
                ...
                if (properties) {
                    ...
                    flattened.push({ schema: structuredClone(value), argSpec });
                }
            }
        }
        return flattened;
...
class SchemaUnsupportedSchemaError extends TypeError {
    ...
    constructor(schema, extType, extName) {
        super((() => {
            ...
            if (support_1.util.isPlainObject(schema)) {
                if (schema.$async) {
