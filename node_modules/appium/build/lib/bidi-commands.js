...
exports.determineBiDiHost = determineBiDiHost;
exports.onBidiConnection = onBidiConnection;
exports.onBidiMessage = onBidiMessage;
exports.onBidiServerError = onBidiServerError;
exports.cleanupBidiSockets = cleanupBidiSockets;
...
const BIDI_EVENTS_MAP = new WeakMap();
...
function determineBiDiHost(address) {
    if (!(0, network_1.isBroadcastIp)(address)) {
        return address;
    }
    const nonLocalInterfaces = (0, network_1.fetchInterfaces)(address === network_1.V4_BROADCAST_IP ? 4 : 6).filter((iface) => !iface.internal);
 ...
 */
function onBidiConnection(ws, req) {
    try {
        const initBiDiSocketFunc = initBidiSocket.bind(this);
        const { bidiHandlerDriver, bidiHandlerPlugins, proxyClient, send, sendToProxy, logSocketErr } = initBiDiSocketFunc(ws, req);
        const initBidiSocketHandlersFunc = initBidiSocketHandlers.bind(this);
        initBidiSocketHandlersFunc(ws, proxyClient, send, sendToProxy, bidiHandlerDriver, bidiHandlerPlugins, logSocketErr);
        if (proxyClient) {
            const initBidiProxyHandlersFunc = initBidiProxyHandlers.bind(bidiHandlerDriver);
            initBidiProxyHandlersFunc(proxyClient, ws, send);
        }
        const initBidiEventListenersFunc = initBidiEventListeners.bind(this);
 ...
 * @param plugins
 */
async function onBidiMessage(data, driver, plugins) {
    let resMessage;
    let id = 0;
    const driverLog = driver.log;
    const dataTruncated = support_1.util.truncateString(data.toString(), { length: MAX_LOGGED_DATA_LENGTH });
    try {
        let method;
        let params;
        try {
            ...
            throw new base_driver_1.errors.InvalidArgumentError(`Could not parse Bidi command '${dataTruncated}': ${message}`);
        }
        ...
        const executeWrappedCommand = wrapCommandWithPlugins(driver, plugins, method, params);
        ...
        if (err !== null &&
            ...
            typeof err.bidiErrObject === 'function') {
            ...
            resMessage = {
                id,
                type: 'error',
                error: base_driver_1.errors.UnknownError.error(),
 ...
 * @param err
 */
function onBidiServerError(err) {
 ...
 * @param sessionId
 */
function cleanupBidiSockets(sessionId) {
...
function wrapCommandWithPlugins(driver, plugins, method, params) {
 ...
 * @param req The connection pathname, which might include the session id
 */
function initBidiSocket(ws, req) {
    ...
    if (sessionMatch) {
        ...
        if (bidiProxyUrl) {
            try {
                ...
                throw new Error(`Got request for ${driverName} to proxy bidi connections to upstream socket with ` +
                    `url ${bidiProxyUrl}, but this was not a valid url`, { cause: e });
            }
            ...
            bidiHandlerPlugins.push(...this.pluginsForSession(sessionId));
        }
    }
    else {
        ...
        bidiHandlerPlugins.push(...this.pluginsForSession());
    }
    const driverLog = bidiHandlerDriver.log;
    const logSocketErr = (err) => {
    ...
    const sendFactory = (socket) => {
        const socketSend = (0, node_util_1.promisify)(socket.send.bind(socket));
        return async (data) => {
            try {
 ...
 * client
 */
function initBidiProxyHandlers(proxyClient, ws, send) {
    ...
    proxyClient.on('close', (code, reason) => {
        driverLog.debug(`Upstream bidi socket closed connection (code ${code}, reason: '${reason}'). ` +
            `Closing proxy connection to client`);
        const intCode = typeof code === 'number' ? code : parseInt(code, 10);
        if (Number.isNaN(intCode) || intCode < MIN_WS_CODE_VAL || intCode > MAX_WS_CODE_VAL) {
            driverLog.warn(`Received code ${code} from upstream socket, but this is not a valid ` +
 ...
 */
function initBidiSocketHandlers(ws, proxyClient, send, sendToProxy, bidiHandlerDriver, bidiHandlerPlugins, logSocketErr) {
    const driverLog = bidiHandlerDriver.log;
    ...
    ws.on('close', (code, reason) => {
        ...
        if (!support_1.util.isEmpty(eventLogCounts)) {
 ...
 * client
 */
function initBidiEventListeners(ws, bidiHandlerDriver, bidiHandlerPlugins, send) {
    ...
    const eventListenerFactory = (extType, ext) => {
        const eventListener = async ({ context, method, params = {}, }) => {
            ...
            if (!method || !params) {
                ...
                `${(0, utils_1.capitalize)(extType)} emitted a bidi event that was malformed. Require method and params keys ` +
                    `(with optional context). But instead received: ${support_1.util.truncateString(JSON.stringify({
                        context,
                        method,
                        params,
            ...
            if (ws.readyState !== ws_1.default.OPEN) {
                ...
                if (ws.readyState > ws_1.default.OPEN) {
                    ...
                    ext.eventEmitter?.removeListener(constants_1.BIDI_EVENT_NAME, eventListener);
                }
                return;
            }
            ...
            if (Array.isArray(eventSubs) && eventSubs.includes(context)) {
                if (method in eventLogCounts) {
                    ...
                    `<-- BIDI EVENT ${method} (context: '${context}', ` +
                        `params: ${support_1.util.truncateString(JSON.stringify(params), { length: MAX_LOGGED_DATA_LENGTH })}). ` +
    ...
    bidiHandlerDriver.eventEmitter.on(constants_1.BIDI_EVENT_NAME, eventListenerFactory('driver', bidiHandlerDriver));
    for (const plugin of bidiHandlerPlugins) {
        // some old plugins might not have the eventEmitter property
        plugin.eventEmitter?.on(constants_1.BIDI_EVENT_NAME, eventListenerFactory('plugin', plugin));
    }
}
async function assertIsOpen(ws, timeoutMs = 5000) {
    ...
    try {
        await new Promise((resolve, reject) => {
            setTimeout(() => reject(new Error(`The BiDi web socket at ${ws.url} did not ` + `open after ${timeoutMs}ms timeout`)), timeoutMs);
