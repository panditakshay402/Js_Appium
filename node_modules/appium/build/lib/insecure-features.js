...
function configureGlobalFeatures() {
    if (this.args.relaxedSecurityEnabled) {
        logger_1.log.info(`Enabling relaxed security. All insecure features will be ` +
            `enabled unless explicitly disabled by --deny-insecure`);
        this.relaxedSecurityEnabled = true;
    }
    else if (!support_1.util.isEmpty(this.args.allowInsecure)) {
        ...
        if (!support_1.util.isEmpty(globalAllowedFeatures)) {
            logger_1.log.info('Explicitly enabling insecure features:');
            globalAllowedFeatures.forEach((a) => logger_1.log.info(`    ${a}`));
        }
    }
    if (support_1.util.isEmpty(this.args.denyInsecure)) {
        return;
    }
    ...
    if (support_1.util.isEmpty(globalDeniedFeatures)) {
        return;
    }
    logger_1.log.info('Explicitly disabling insecure features:');
    globalDeniedFeatures.forEach((a) => logger_1.log.info(`    ${a}`));
}
/**
...
function configureDriverFeatures(driver, driverName) {
    ...
    if (!support_1.util.isEmpty(allowedDriverFeatures)) {
    ...
    if (support_1.util.isEmpty(deniedDriverFeatures)) {
        return;
    }
