...
(0, utils_1.setBlocking)(true);
class Log extends node_events_1.EventEmitter {
    level;
    prefixStyle;
    headingStyle;
    heading;
    stream; // Defaults to process.stderr; set to null when using custom output (e.g. Winston)
    _asyncStorage;
    _colorEnabled;
    _buffer;
    _style;
    ...
    updateAsyncStorage(contextInfo, replace) {
        if (!(0, utils_1.isPlainObject)(contextInfo)) {
            return;
        }
        if (replace) {
    ...
    _format(msg, style = {}) {
        ...
        if (this.useColor()) {
            ...
            if (settings.length) {
                output += (0, utils_1.ansiColor)(...settings);
            }
            if (style.bell) {
                output += (0, utils_1.ansiBeep)();
            }
        }
        output += msg;
        if (this.useColor()) {
            output += (0, utils_1.ansiColor)('reset');
        }
        return output;
    }
    write(msg, style = {}) {
...
const g = globalThis;
exports.GLOBAL_LOG = g[GLOBAL_NPMLOG_KEY] ??
    (() => {
        const log = new Log();
