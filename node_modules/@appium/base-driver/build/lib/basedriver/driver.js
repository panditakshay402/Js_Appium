...
const extension_command_name_1 = require("../helpers/extension-command-name");
...
class BaseDriver extends core_1.DriverCore {
    ...
    get _desiredCapConstraints() {
        return Object.freeze((0, utils_1.mergePlainObjects)({}, types_1.BASE_DESIRED_CAP_CONSTRAINTS, this.desiredCapConstraints));
    }
    /**
    ...
    async executeCommand(cmd, ...args) {
        ...
        const runCommandPromise = async () => {
            ...
            try {
                ...
                // intentionally
                if (!wasSessionShutdownUnexpectedly &&
                    this.isCommandsQueueEnabled &&
                    cmd !== protocol_1.DELETE_SESSION_COMMAND) {
        ...
        const commandsQueueGuard = this.commandsQueueGuard;
        const commandsQueueLen = commandsQueueGuard.queues?.[synchronizationKey]?.length ?? 0;
        if (this.isCommandsQueueEnabled && commandsQueueLen > 0) {
            this.log.debug(`Scheduling the '${cmd}' command to the ${this.constructor.name} commands queue. ` +
                `${support_1.util.pluralize('queue item', commandsQueueLen, true)} ${commandsQueueLen === 1 ? 'is' : 'are'} ` +
    ...
    clarifyCommandName(cmd, args) {
        if (cmd === 'execute') {
            ...
            if (typeof firstArg === 'string' && firstArg.trim().length > 0) {
                return extension_command_name_1.resolveExecuteExtensionName.call(this, firstArg);
            }
        }
        return cmd;
    }
    ...
    driverData) {
        ...
        this.log.debug();
        const originalCaps = structuredClone([w3cCapabilities, w3cCapabilities1, w3cCapabilities2].find(capabilities_1.isW3cCaps));
        if (!originalCaps) {
        ...
        try {
            ...
            throw new protocol_1.errors.SessionNotCreatedError(message);
        }
        ...
        this.log.updateAsyncContext({
            sessionId: this.sessionId,
            sessionSignature: (0, session_1.calcSignature)(this.sessionId),
        });
    ...
    logExtraCaps(caps) {
        ...
        if (extraCaps.length) {
            ...
            for (const cap of extraCaps) {
                const suggestion = (0, levenshtein_match_1.getLevenshteinSuggestion)(cap, knownCaps);
    ...
    validateDesiredCaps(caps) {
        ...
        try {
            ...
            throw this.log.errorWithException(new protocol_1.errors.SessionNotCreatedError(`Session capabilities were not valid for the ` +
                `following reason(s): ${capError.message}`, capError));
        }
