...
class DriverCore extends extension_core_1.ExtensionCore {
    ...
    isFeatureEnabled(name) {
        ...
        if (!support_1.util.isEmpty(this.denyInsecure) && parseFullNames(this.denyInsecure).some(matches)) {
            return false;
        }
        // if we specifically have allowed the feature, return true
        if (!support_1.util.isEmpty(this.allowInsecure) && parseFullNames(this.allowInsecure).some(matches)) {
            return true;
        }
    ...
    proxyRouteIsAvoided(sessionId, method, url, body) {
        for (const avoidSchema of this.getProxyAvoidList(sessionId)) {
            ...
            const normalizedUrl = url.replace(new RegExp(`^${support_1.util.escapeRegExp(this.basePath)}`), '');
