    ...
    if (isHttpUploadOptions(uploadOptions, url)) {
        if (!uploadOptions.fileFieldName) {
            uploadOptions.headers = {
                ...((0, util_1.isPlainObject)(uploadOptions.headers) ? uploadOptions.headers : {}),
                'Content-Length': size,
            };
        }
...
async function downloadFile(remoteUrl, dstPath, downloadOptions = {}) {
    ...
    if ((0, util_1.isPlainObject)(headers)) {
...
function toAxiosAuth(auth) {
    if (!auth || !(0, util_1.isPlainObject)(auth)) {
        return null;
    }
...
async function uploadFileToHttp(localFileStream, parsedUri, uploadOptions = {}) {
    ...
    if (fileFieldName) {
        ...
        if (formFields) {
            ...
            else if ((0, util_1.isPlainObject)(formFields)) {
        ...
        requestOpts.headers = {
            ...((0, util_1.isPlainObject)(headers) ? headers : {}),
            ...form.getHeaders(),
        };
    ...
    logger_1.default.debug(`Performing ${method} to ${href} with options (excluding data): ` +
        JSON.stringify((() => {
            const requestOptsWithoutData = { ...requestOpts };
