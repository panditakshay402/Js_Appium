...
async function buildSite({ mkdocsYml: mkDocsYmlPath, siteDir, cwd = process.cwd(), serve = false, serveOpts, execOpts, } = {}) {
    const stop = (0, utils_1.stopwatch)('build-mkdocs');
    const pythonPath = await (0, fs_1.requirePython)();
    ...
    if (serve) {
        ...
        let relSiteDir;
        if (siteDir) {
            relSiteDir = (0, utils_1.relative)(cwd, siteDir);
        }
        else {
            ({ site_dir: siteDir } = await (0, fs_1.readMkDocsYml)(mkDocsYmlPath));
            if (siteDir) {
                log.debug('Found site_dir %s', siteDir);
                relSiteDir = (0, utils_1.relative)(node_path_1.default.dirname(mkDocsYmlPath), siteDir);
            }
            else {
                ...
                relSiteDir = (0, utils_1.relative)(cwd, constants_1.DEFAULT_SITE_DIR);
            }
        }
...
async function doServe(pythonPath, args = [], opts = {}) {
    ...
    return (0, utils_1.spawnBackgroundProcess)(pythonPath, finalArgs, opts);
}
/**
 * Runs `mkdocs build`
 * @param pythonPath Path to Python 3 executable
...
async function doBuild(pythonPath, args = [], opts = {}) {
    ...
    log.debug('Executing %s via: %s, %O', constants_1.NAME_MKDOCS, pythonPath, finalArgs);
    return await (0, utils_1.execWithErrorHandling)(pythonPath, finalArgs, opts);
}
